...
bool get_ops(const char* filename) {
	...
	while (!feof(infile)) {
		...
		if (strcmp(key,"nuclei_filename") == 0) {
			strncpy(ops.nuclei_filename,val,sizeof(ops.nuclei_filename));
		} else if (strcmp(key,"reaclib_filename") == 0) {
			strncpy(ops.reaclib_filename,val,sizeof(ops.reaclib_filename));
		} else if (strcmp(key,"output_filename") == 0) {
			strncpy(ops.output_filename,val,sizeof(ops.output_filename));
